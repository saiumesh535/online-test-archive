...
fn write_component(dir_name: &String, file_name: &String) {
    let mut content = format!("{}", String::from_utf8_lossy(include_bytes!("components/cli.component.tsx")));
    let component = format!("{}.tsx", file_name);
    let component_name = format!("{file_name}Component", file_name = file_name);
    content = string_replace("ReactComponent", component_name.as_str(), 2, content);
...
fn write_effect(dir_name: &String, file_name: &String) {
    let mut content = format!("{}", String::from_utf8_lossy(include_bytes!("components/cli.effects.ts")));
...
fn write_actions(dir_name: &String, file_name: &String) {
    let mut content = format!("{}", String::from_utf8_lossy(include_bytes!("components/cli.actions.ts")));
...
fn write_reducer(dir_name: &String, file_name: &String) {
    let mut content = format!("{}", String::from_utf8_lossy(include_bytes!("components/cli.reducer.ts")));
    let component = format!("{}.reducer.ts", file_name);
    content = string_replace("%file_name%", file_name.as_str(), 9, content);
    match write_file_src(dir_name, &component, content) {
